...
*   Added `sha256` and `sha256_src` attributes to `maven_jar`. Please consider migrating to SHA-256 as SHA-1 has been deemed [cryptographically insecure](http://shattered.io). Or, use [`rules_jvm_externaGenrule now supports `cmd_bash`, `cmd_ps`, `cmd_bat` attributes. More details in [`genrule` documentation](https://docs.bazel.build/versions/master/be/general.html#genrule.cmd)l`](https://github.com/bazelbuild/rules_jvm_external) to manage your transitive Maven dependencies with artifact pinning and SHA-256 verification support.
